...
for (let i=0;i<900;i+=3){sa[i]=(Math.random()-0.5)*1000;sa[i+1]=Math.random()*400+50;sa[i+2]=(Math.random()-0.5)*1000;}
...
const rw = new THREE.Mesh(new THREE.PlaneGeometry(10,80),new THREE.MeshStandardMaterial({color:0x444444,roughness:0.8}));
rw.rotation.x = -Math.PI/2; rw.position.set(0,-4.98,35); scene.add(rw);
for (let i=0;i<=70;i+=3){
  const st = new THREE.Mesh(new THREE.PlaneGeometry(2.5,0.3),new THREE.MeshStandardMaterial({color:0xffffff}));
...
for (let i=0;i<60;i++){
  ...
  while(Math.abs(x)<12 && z>0 && z<70){x=(Math.random()-0.5)*350; z=(Math.random()-0.5)*350;}
...
controls.minDistance = 5; controls.maxDistance = 80; controls.maxPolarAngle = Math.PI*0.8;
