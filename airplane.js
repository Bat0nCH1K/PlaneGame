...
const fm = new THREE.MeshStandardMaterial({color:0x4488cc,roughness:0.4,metalness:0.6});
const fuselage = new THREE.Mesh(fg, fm);
fuselage.rotation.x = Math.PI/2;
fuselage.name = 'fuselage';
airplaneGroup.add(fuselage);

const ng = new THREE.SphereGeometry(0.5,10,10,0,Math.PI*2,0,Math.PI/2);
const nose = new THREE.Mesh(ng, new THREE.MeshStandardMaterial({color:0x5599dd,roughness:0.3,metalness:0.7}));
nose.position.z = -1.6;
...
const cg = new THREE.SphereGeometry(0.4,10,10);
const cockpit = new THREE.Mesh(cg, new THREE.MeshStandardMaterial({color:0x88ccff,roughness:0.1,metalness:0.3,transparent:true,opacity:0.7}));
...
window.createPart = function(type) {
  ...
  if (type==='wing') {
    ...
    const eg = new THREE.BoxGeometry(0.06,0.1,0.72);
    const em = new THREE.MeshStandardMaterial({color:0xc0560a});
    ...
    g.userData.snapPos = new THREE.Vector3(0.55,0,0);
  }
  const lb = label(type==='wing'?'🪽':type==='tail'?'🪁':'🚀');
...
window.clearAllParts = function() {
  ...
  window.scene.children.forEach(c => {
    if (c.userData?.isPart && c!==airplaneGroup && c!==partsLayer && !airplaneGroup.children.includes(c)) rm.push(c);
  });
  rm.forEach(c => {
    c.traverse(x => {
      if (x.geometry) x.geometry.dispose();
      if (x.material) { Array.isArray(x.material) ? x.material.forEach(m=>m.dispose()) : x.material.dispose(); }
    });
...
window.selectPart = function(part) {
  if (window.selectedPart && window.selectedPart !== part) {
    window.selectedPart.traverse(c => {
      if (c.userData?.isPartMesh && c.material?.emissive) c.material.emissive.set(0x000000);
    });
  }
  window.selectedPart = part;
  if (part) {
    part.traverse(c => {
      if (c.userData?.isPartMesh && c.material?.emissive) c.material.emissive.set(0x444444);
    });
...
let dragged = null, plane = new THREE.Plane(new THREE.Vector3(0,1,0),0), offset = new THREE.Vector3(), clickOnPart = false;
...
function hits(e) {
  ...
  return {
    fh: raycaster.intersectObjects(hitTargets, false),
    ph: raycaster.intersectObjects(window.placedParts, true)
  };
}
...
window.renderer.domElement.addEventListener('pointerdown', e => {
  ...
  if (ph.length) {
    ...
    if (o && window.placedParts.includes(o)) {
      ...
      offset.copy(wp).sub(pt);
      const n = new THREE.Vector3(0,0,1).applyQuaternion(window.camera.quaternion);
      plane.set(n, 0);
      clickOnPart = true;
...
window.renderer.domElement.addEventListener('pointermove', e => {
  ...
  if (raycaster.ray.intersectPlane(plane, t)) {
    t.add(offset);
    window.airplaneGroup.worldToLocal(t);
...
window.renderer.domElement.addEventListener('pointerup', e => {
  if (!dragged) {
    if (!clickOnPart && !window.isFlying) placeClick(e);
    return;
  }
  const snap = dragged.userData.snapPos;
  if (snap) {
    const ws = snap.clone();
    window.airplaneGroup.localToWorld(ws);
    const pw = new THREE.Vector3();
    dragged.getWorldPosition(pw);
    if (pw.distanceTo(ws) < 1.2) dragged.position.copy(snap);
  }
  dragged = null;
